...
# Structure hiérarchique AHP à 2 niveaux
st.subheader("🔢 Niveau 1 : Comparaison des Facettes principales")
st.markdown("**Comparez : Product satisfaction, Supply innovation capability, Service level**")

facettes = ["Product satisfaction", "Supply innovation capability", "Service level"]
matrice_facettes = np.ones((3, 3))

col1, col2, col3 = st.columns(3)
with col1:
    val_12 = st.number_input(
        "Product satisfaction vs Supply innovation",
        min_value=0.111, max_value=9.0, value=1.0, step=0.5,
        key="facet_12"
    )
    matrice_facettes[0, 1] = val_12
    matrice_facettes[1, 0] = 1 / val_12

with col2:
    val_13 = st.number_input(
        "Product satisfaction vs Service level",
        min_value=0.111, max_value=9.0, value=1.0, step=0.5,
        key="facet_13"
    )
    matrice_facettes[0, 2] = val_13
    matrice_facettes[2, 0] = 1 / val_13

with col3:
    val_23 = st.number_input(
        "Supply innovation vs Service level",
        min_value=0.111, max_value=9.0, value=1.0, step=0.5,
        key="facet_23"
    )
    matrice_facettes[1, 2] = val_23
    matrice_facettes[2, 1] = 1 / val_23
...
# Niveau 2 : Comparaison des critères au sein de chaque facette
st.subheader("🔢 Niveau 2 : Comparaison des Critères par Facette")

# Groupe 1 : Product satisfaction (C1, C2, C3)
st.markdown("### 📦 Product satisfaction")
...
# Groupe 2 : Supply innovation capability (C4, C5)
st.markdown("### 🚀 Supply innovation capability")
